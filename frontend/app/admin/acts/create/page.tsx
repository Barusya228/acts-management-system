...
function ActCreatePageContent() {
  ...
  const handleDeviceSelect = (serial: string) => {
    ...
    const device = availableDevices.find(d => d.serial_number === serial);
    if (device) {
      setFormData(prev => ({ ...prev, item_name: device.name, item_serial: device.serial_number }));
    } else {
      setFormData(prev => ({ ...prev, item_name: '', item_serial: '' }));
    }
  };
                ...
                {selectedDeviceSerial ? (
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="rounded-lg bg-white px-3 py-1.5 text-xs font-medium text-slate-600 shadow-sm ring-1 ring-blue-200 transition hover:bg-red-50 hover:text-red-600"
                    >
                      ✕ Убрать
                    </button>
                  </div>
                  ...
                  <>
                    ...
                    <select
                      ...
                      className="w-full rounded-lg border border-blue-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
                    >
                    ...
                    <p className="mt-1 text-xs text-slate-400">или заполните поля вручную ниже</p>
                  </>
                )}
              </div>
            )}
...
export default function ActCreatePage() {
  return (
    <Suspense fallback={<AdminLayout><div className="mx-auto max-w-3xl py-10 text-center">Загрузка...</div></AdminLayout>}>
