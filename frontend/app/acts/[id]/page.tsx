...
export default function ActViewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-[#eef2f6] text-slate-900">
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex min-h-16 max-w-7xl items-center gap-2 px-3 sm:gap-3 sm:px-4"><Link href={user?.role === 'ADMIN' ? '/admin/acts' : '/guest'} className="flex h-11 shrink-0 items-center rounded-xl px-2 text-sm font-semibold text-slate-500 hover:bg-slate-100 sm:px-3">←<span className="hidden sm:inline"> К актам</span></Link><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-blue-600">Приём-передача</p><h1 className="truncate text-base font-black sm:text-lg">Акт выдачи техники · ACT-{act.id.split('-')[0].toUpperCase()}</h1></div>{user?.role === 'ADMIN' && <Link href={`/admin/acts/${act.id}`} className="ml-auto flex min-h-11 shrink-0 items-center rounded-xl bg-slate-900 px-3 text-sm font-bold text-white sm:px-4">Управление</Link>}</div></header>
    <main className="mx-auto grid max-w-7xl gap-4 p-3 sm:p-4 lg:grid-cols-[46%_54%] lg:gap-5 lg:p-5">
      <section className="space-y-4 lg:max-h-[calc(100vh-104px)] lg:overflow-y-auto lg:pr-1">
        <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div className="flex flex-wrap items-start justify-between gap-3"><div><span className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${status.tone}`}>{status.label}</span><p className="mt-3 max-w-xl text-sm leading-6 text-slate-500">{status.detail}</p></div><div className="text-right"><p className="text-xs text-slate-400">Дата выдачи</p><p className="font-black">{new Date(act.issue_date).toLocaleDateString('ru-RU')}</p></div></div><div className="mt-5 grid grid-cols-3 items-start gap-2 sm:grid-cols-[1fr_auto_1fr_auto_1fr] sm:items-center"><Progress active label={`Получатели ${signedCount}/${recipients.length}`} /><span className="hidden h-px w-full bg-slate-200 sm:block"/><Progress active={act.status !== 'DRAFT'} label="Подтверждение IT"/><span className="hidden h-px w-full bg-slate-200 sm:block"/><Progress active={act.status === 'COMPLETED' || isReturn} label="Завершено"/></div></div>
    ...
    </main>{returnConfirmOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><div className="w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl"><p className="text-xs font-bold uppercase tracking-widest text-violet-600">Подтверждение</p><h2 className="mt-1 text-xl font-black">Начать возврат всего комплекта?</h2><p className="mt-2 text-sm text-slate-500">После начала потребуется подпись IT и получателя.</p><div className="mt-4 max-h-64 space-y-2 overflow-y-auto rounded-2xl bg-slate-50 p-4"><BundleItem color="bg-blue-600" badge="Основное" name={act.item_name} detail={`Инв: ${act.item_serial || '—'}`} />{extraDevices.map((item,index) => <BundleItem key={index} color="bg-violet-500" badge="Дополнительное" name={String(item.name || 'Устройство')} detail={`Инв: ${String(item.serial || '—')}`} />)}{accessories.map((item,index) => <BundleItem key={index} color="bg-emerald-500" badge="Мелкая техника" name={String(item.name || 'Позиция')} detail={`${Number(item.quantity || 1)} шт.`} />)}</div><div className="mt-5 flex gap-3"><button disabled={busy} onClick={() => { setReturnConfirmOpen(false); startReturn(); }} className="min-h-12 flex-1 rounded-xl bg-violet-600 font-black text-white">Начать возврат</button><button onClick={() => setReturnConfirmOpen(false)} className="min-h-12 rounded-xl bg-slate-100 px-4 font-bold">Отмена</button></div></div></div>}{versionsOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><div className="flex max-h-[90dvh] w-full max-w-xl flex-col rounded-3xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase text-blue-600">ACT-{act.id.split('-')[0].toUpperCase()}</p><h2 className="text-xl font-black">История версий</h2></div><button onClick={() => setVersionsOpen(false)} className="min-h-11 rounded-xl bg-slate-100 px-4">Закрыть</button></div><div className="mt-4 min-h-0 flex-1 space-y-2 overflow-y-auto">{versions.map(version => <button key={version.id} onClick={() => { setVersionsOpen(false); openPdf(version.version_number); }} className="flex min-h-14 w-full items-center justify-between rounded-xl bg-slate-50 px-4 text-left hover:bg-blue-50"><span><span className="block font-bold">Версия {version.version_number}</span><span className="text-xs text-slate-500">{version.change_note || 'Состояние документа'} · {new Date(version.created_at).toLocaleString('ru-RU')}</span></span><span className="text-sm font-bold text-blue-600">Открыть PDF</span></button>)}</div></div></div>}{pdfOpen && pdfUrl && <div className="fixed inset-0 z-50 bg-slate-950/70 p-2 sm:p-4"><div className="mx-auto flex h-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white"><div className="flex flex-wrap items-center justify-between gap-2 border-b p-3"><h2 className="font-black">Предпросмотр PDF</h2><div className="flex gap-2"><a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center rounded-xl bg-blue-600 px-4 text-sm font-bold text-white">Новая вкладка</a><button onClick={() => setPdfOpen(false)} className="min-h-11 rounded-xl bg-slate-100 px-4">Закрыть</button></div></div><iframe src={pdfUrl} className="min-h-0 flex-1" title="PDF акта" /></div></div>}
  </div>;
}

function Progress({ active, label }: { active: boolean; label: string }) { return <div className="text-center"><span className={`mx-auto block h-3 w-3 rounded-full ${active ? 'bg-blue-600 ring-4 ring-blue-100' : 'bg-slate-200'}`}/><span className="mt-2 block text-[11px] font-bold leading-tight text-slate-500 sm:text-[10px]">{label}</span></div>; }
